...
func loadBenchmarkData(b *testing.B) ([]byte, error) {
	benchmarkOnce.Do(func() {
		input := loadDistTopo(b)
...
func BenchmarkLookupTransport(b *testing.B) {
	...
	run := func(b *testing.B, r *embedbin.Reader) {
		b.ReportAllocs()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
	...
	b.Run("file-reader-at", func(b *testing.B) {
		...
		r, err := embedbin.OpenReaderAt(file, int64(len(data)))
		if err != nil {
			b.Fatal(err)
		}
		run(b, r)
	...
	b.Run("sector-cache", func(b *testing.B) {
		...
		r, err := embedbin.OpenReaderAt(source, int64(len(data)))
		if err != nil {
			b.Fatal(err)
		}
