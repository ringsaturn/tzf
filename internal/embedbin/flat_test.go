...
func openMFixture(t *testing.T, input *pb.CompressedTopoTimezones, opts EncodeOptions) ([]byte, *Reader) {
	t.Helper()
...
func TestProfileGuards(t *testing.T) {
	...
	if _, _, err := mReader.Lookup(5, 5); !errors.Is(err, ErrProfile) {
		t.Fatalf("M Lookup error = %v", err)
	}
	if _, err := mReader.LookupInto(5, 5, make([]int32, 0, 1)); !errors.Is(err, ErrProfile) {
		t.Fatalf("M LookupInto error = %v", err)
	}
	if _, err := mReader.Expand(); !errors.Is(err, ErrProfile) {
		t.Fatalf("M Expand error = %v", err)
	}
	if _, err := eReader.Flat(); !errors.Is(err, ErrProfile) {
		t.Fatalf("E Flat error = %v", err)
	}
	if err := VerifyM(fixture("Etc/Test"), eReader); !errors.Is(err, ErrProfile) {
	...
	unknown[profileOffset] = 2
	rechecksum(unknown)
	if _, err := Open(unknown); err == nil {
		t.Fatal("Open accepted unknown profile")
	}
	_ = mData
}
...
func TestMRejectsStructuralCorruption(t *testing.T) {
	...
	tests := []struct {
		...
		{"ring partition", func(v []byte) {
			o := findSectionEntry(v, sectionFlatRingDir)
		...
		{"flatpoints truncated", func(v []byte) {
			o := findSectionEntry(v, sectionFlatPoints)
			length := binary.LittleEndian.Uint32(v[o+8:])
			binary.LittleEndian.PutUint32(v[o+8:], length-8)
			rechecksum(v)
		}},
		{"E section in M file", func(v []byte) {
			o := findSectionEntry(v, sectionGrid)
			binary.LittleEndian.PutUint32(v[o:], sectionPoints)
			rechecksum(v)
		}},
	}
	...
	o := findSectionEntry(mutated, sectionGrid)
	binary.LittleEndian.PutUint32(mutated[o:], sectionFlatPoints)
	rechecksum(mutated)
	if _, err := Open(mutated); err == nil {
		t.Fatal("Open accepted M section in E file")
	}
}

// TestVerifyMSemanticTrustBoundary mirrors the E-profile deterministic
// encoding test: a coordinate flip is structurally valid (Open accepts it)
...
func TestVerifyMSemanticTrustBoundary(t *testing.T) {
	...
	o := findSectionEntry(mutated, sectionFlatPoints)
