...
func openFuzzyFixture(t *testing.T, pre *pb.PreindexTimezones, names ...string) ([]byte, *Reader) {
	t.Helper()
...
func fuzzySectionEntry(t *testing.T, data []byte) (off, length uint32, entry int) {
	...
	for i := 0; i < count; i++ {
		...
		if binary.LittleEndian.Uint32(data[o:]) == sectionFuzzy {
...
func TestFuzzyReaderRejectsStructuralCorruption(t *testing.T) {
	...
	tests := []struct {
		...
		{"unknown profile", func(v []byte) { v[profileOffset] = 2 }},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if _, err := Open(mutated); err == nil {
				t.Fatal("Open accepted corrupted FUZZY data")
			}
		})
	}
}
...
func TestFuzzySectionAlignmentAndMinorVersion(t *testing.T) {
	...
	if data[4] != formatMajor || data[5] != formatMinor {
		t.Fatalf("format version = %d.%d", data[4], data[5])
	}
	if data[profileOffset] != profileE {
		t.Fatalf("profile byte = %d", data[profileOffset])
	}
}
...
func TestFuzzyAbsentFromPlainFile(t *testing.T) {
	...
	if _, _, err := r.FuzzyLookup(10, 10); err != ErrNoFuzzy {
